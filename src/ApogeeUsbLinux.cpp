...
//	struct usb_dev_;
	...
	for(bus = usb_busses; bus && !found; bus = bus->next) {
		for(dev = bus->devices; dev && !found; dev = dev->next) {
			...
			// Test avec un device different (trouvé avec la commadne lsusb) 413c:2106
			//if (dev->descriptor.idVendor == 0x413c &&
			//		dev->descriptor.idProduct == 0x2106) {
				found = found+1;
				if (found == DevNumber) {
					hDevice = usb_open(dev); // Erreur ici
					firmwareRevision = dev->descriptor.bcdDevice;
					std::cerr << "Found ALTA USB. Attempting to open... ";
					if (hDevice) { // pointer not NULL => it worked
						if (!usb_get_string_simple(hDevice,
								dev->descriptor.iSerialNumber,
								string, sizeof(string))) {
							std::cerr << "Error while opening camera!!!!!!!!" <<endl;
							exit(1);
						}
						std::cerr << "Success.\n";
						std::cerr << "Serial number: " << string << endl;
					}
...
{ 
	...
	*/ 
        ...
        if (firmwareRevision >= APUSB_CUSTOM_SN_DID_SUPPORT) {
...
{ 
	...
	if ( firmwareRevision >=  APUSB_8051_REV_DID_SUPPORT) {
