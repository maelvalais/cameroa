...
//#include <stdio.h> // Mael Valais: add because sprintf won't be valid in AltaDebugPrint definition
#define AltaDebugOutputString(...) fprintf(stderr,__VA_ARGS__)
#else
#define AltaDebugString(__X__) OutputDebugString(__X__)
#endif

#define AltaDebugPrint( __A__, __B__, __C__ ) sprintf( __A__, __B__, __C__)
//#define AltaDebugPrint(__STRING__,...) sprintf(__STRING__,__VA_ARGS__)
#else
